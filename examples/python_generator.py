...
def generate_gimmecv_url(
    markdown_cv: str,
    base_url: str = "https://your-gimmecv-instance.com",
    ...
    Args:
        ...
        options: Optional layout options dict to embed (v2). Omit for v1 (content-only).

    Returns:
        Dictionary with URL and statistics.
    """
    # Step 1: Build the payload
    # Pass options to embed layout config (v2). Omit or pass None for v1.
    payload = (
        json.dumps({"v": 2, "c": markdown_cv, "o": options}).encode("utf-8")
        if options
        else markdown_cv.encode("utf-8")
    )
    ...
    base64_url = base64_str.replace("+", "-").replace("/", "_").replace("=", "")
